...
def init_db():
    ...
    # classes + class_students: pre-registered classes (SS1..SS3) and their student lists
    ...
    ensure_column('sessions', 'class', 'TEXT')   # store student's class in session
    ...
    # ensure some default classes exist (safe to call multiple times)
    try:
        for cname in ('SS1','SS2','SS3'):
            c.execute('INSERT OR IGNORE INTO classes (id,name) VALUES (?,?)', (str(uuid.uuid4())[:8], cname))
        conn.commit()
    except Exception:
        pass
...
def create_exam():
    ...
    # allow admin to optionally assign a teacher_id when creating exam
    teacher_id = (data.get('teacher_id') or '').strip() or None
    exam_id = str(uuid.uuid4())[:8]
    conn = db_conn(); c = conn.cursor()
    # if teacher_id invalid, ignore it
    if teacher_id:
        c.execute('SELECT id FROM teachers WHERE id=?', (teacher_id,))
        if not c.fetchone():
            teacher_id = None
    c.execute('INSERT INTO exams (id,title,duration_minutes,tag,teacher_id) VALUES (?,?,?,?,?)', (exam_id, title, duration, tag, teacher_id))
...
def start_exam():
    ...
    student_class = (data.get('class') or data.get('student_class') or '').strip()
    if not exam_id:
    ...
    # ensure student provided (require selection)
    if not student_name:
        conn.close(); return jsonify({'error': 'student_name required'}), 400

    # prevent duplicate submissions: if there's already a result tied to this exam & student_name
    try:
        c.execute('SELECT 1 FROM results r JOIN sessions s ON r.token=s.token WHERE s.exam_id=? AND LOWER(s.student_name)=LOWER(?)', (exam_id, student_name))
        if c.fetchone():
            conn.close(); return jsonify({'error': 'already_submitted', 'message': 'Student has already submitted this exam'}), 403
    except Exception:
        # fall through if DB check fails
        pass
    ...
    c.execute('INSERT INTO sessions (token, exam_id, start_time, end_time, student_name, question_state, tag, class) VALUES (?,?,?,?,?,?,?,?)',
              (token, exam_id, start_time, end_time, student_name or None, json.dumps(question_state), exam_tag, student_class or None))
    conn.commit(); conn.close()
...
def submit(token):
    """
    Submit answers for a session token.
    Use the student_name stored in the sessions row (ignore client-supplied name to avoid
    accidental reuse/spoofing). Compute score from the per-session question_state and
    persist answers_detail and name into results.
    """
    data = request.json or {}
    ...
    # Prefer the session-stored student name always (prevents a client reusing a different name).
    session_name = row['student_name'] if 'student_name' in row.keys() and row['student_name'] else None
    name = session_name or (data.get('name') or '').strip()
    ...
    # Replace any prior result for this token and persist new result including the session name.
...
def upload_students():
    exam_id = request.form.get('exam_id')
    file = request.files.get('file')
    class_name = (request.form.get('class') or '').strip()  # optional target class (SS1/SS2/SS3)
    if not exam_id or not file:
        return jsonify({'error': 'Missing exam_id or file'}), 400
    conn = db_conn(); c = conn.cursor()
    ...
    for name in names:
        ...
        # also attach to class if requested
        if class_name:
            try:
                csid = str(uuid.uuid4())[:8]
                c.execute('INSERT INTO class_students (id,class_name,student_name) VALUES (?,?,?)', (csid, class_name, name))
            except Exception:
                pass
        inserted += 1
    conn.commit(); conn.close()
...
def login_teacher():
    ...
    ph = _hash_password(password)
    conn = db_conn(); c = conn.cursor()
    c.execute('SELECT id, token, approved FROM teachers WHERE name=? AND password_hash=?', (name, ph))
    row = c.fetchone()
    if not row:
        conn.close()
    ...
    if not approved:
        conn.close()
        return jsonify({'error': 'teacher_not_approved', 'message': 'Account pending admin approval'}), 403
    token = row['token']
    if not token:
        # generate a stable token for this teacher
        token = str(uuid.uuid4())[:24]
        try:
            c.execute('UPDATE teachers SET token=? WHERE id=?', (token, row['id']))
            conn.commit()
        except Exception:
            pass
    conn.close()
...
def download_subject():
    """
    Download results. Prefer exam_id when provided (returns only that exam's results).
    Query args:
      - exam_id=<id>        (preferred — returns results for that exam)
      - subject=<name>      (fallback: returns results matching teacher.subject)
      - format=xlsx|csv
    """
    ...
    try:
        if exam_id:
            # resolve exam metadata (title/subject/tag) and use exam_id filter
            c.execute('SELECT e.title, e.tag, COALESCE(t.subject, \'\') AS subject FROM exams e LEFT JOIN teachers t ON e.teacher_id=t.id WHERE e.id=?', (exam_id,))
            er = c.fetchone()
            if not er:
                conn.close(); return jsonify({'error': 'exam not found'}), 404
        ...
        # keep connection open until we fetch, so move fetch below
        pass

    # avoid returning an unrelated cached file: build file names that include exam_id when present
    ...
    for r in rows:
        ...
        data.append({
            ...
            'subject': (r['subject'] or '') if 'subject' in r.keys() else '',
            'tag': (r['tag'] or '') if 'tag' in r.keys() else '',
            'token': r['token'] if 'token' in r.keys() else '',
            'name': (r['name'] or '') if 'name' in r.keys() else '',
    ...
    si = io.StringIO()
    writer = csv.writer(si)
    # only export subject, student name and score
    writer.writerow(['subject', 'name', 'score'])
    for d in data:
        writer.writerow([d.get('subject',''), d.get('name',''), d.get('score','')])
    mem = io.BytesIO(); mem.write(si.getvalue().encode('utf-8')); mem.seek(0)
...
@app.route('/api/list_classes')
def api_list_classes():
    """Return all classes (id, name)."""
    conn = db_conn(); c = conn.cursor()
    try:
...
def api_create_class():
    """
    Create a class. Accepts JSON { "name": "SS1", "admin_password": "..." }.
    Uses is_admin_request() so admin_password can be provided in JSON for non-session calls.
    """
    data = request.get_json(silent=True) or {}
    name = (data.get('name') or '').strip()
    if not name:
    ...
    conn = db_conn(); c = conn.cursor()
    try:
        c.execute('INSERT OR IGNORE INTO classes (id, name) VALUES (?,?)', (cid, name))
        conn.commit()
        c.execute('SELECT id, name FROM classes WHERE name=?', (name,))
        row = c.fetchone()
        if row:
...
@app.route('/api/add_class_student', methods=['POST'])
def api_add_class_student():
    """
    Admin adds a student to a class.
    JSON: { "class": "SS1", "class_id": "...", "student_name": "Alice", "admin_password": "..." }
    """
    data = request.get_json(silent=True) or {}
    ...
    try:
        if class_id and not class_name:
            ...
            class_name = rr['name']
        if not class_name:
            return jsonify({'error': 'class or class_id required'}), 400
        # avoid duplicate student rows for same class+name
        c.execute('SELECT id FROM class_students WHERE class_name=? AND LOWER(student_name)=LOWER(?)', (class_name, student_name))
        if c.fetchone():
            return jsonify({'ok': True, 'note': 'already_added'})
        csid = str(uuid.uuid4())[:8]
        c.execute('INSERT INTO class_students (id, class_name, student_name) VALUES (?,?,?)', (csid, class_name, student_name))
        conn.commit()
        return jsonify({'ok': True, 'id': csid, 'class': class_name, 'student_name': student_name})
    finally:
        conn.close()

@app.route('/api/list_class_students')
def api_list_class_students():
    """
    List students in a class.
    Query args: ?class=SS1  or ?class_id=<id>
    Returns: [{id, name}, ...]
    """
    ...
    try:
        ...
        if not class_name:
            return jsonify([])
        c.execute('SELECT id, student_name FROM class_students WHERE class_name=? ORDER BY student_name', (class_name,))
        rows = c.fetchall()
        out = [{'id': r['id'], 'name': r['student_name']} for r in rows]
    finally:
        conn.close()
    return jsonify(out)

@app.template_filter('datetime')
def _fmt_datetime(ts, fmt='%Y-%m-%d %H:%M'):
    """Format unix timestamp or datetime objects for templates."""
    try:
        if ts is None:
            return ''
        ...
        s = str(ts).strip()
        if s.isdigit():
            return datetime.fromtimestamp(int(s)).strftime(fmt)
        return s
    except Exception:
        return str(ts)
